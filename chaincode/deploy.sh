...
if [ $? -eq 0 ]; then
    ...
    echo "  ./network.sh cc query -c ${CHANNEL_NAME} -ccn ${CC_NAME} -ccqc '{\"function\":\"ReadDonateProject\",\"Args\":[\"1\"]}'"
    echo ""
    echo "To create a project:"
    echo "  ./network.sh cc invoke -c ${CHANNEL_NAME} -ccn ${CC_NAME} -ccic '{\"function\":\"CreateDonateProject\",\"Args\":[\"3\",\"TestProject\",\"Description\",\"owner\"]}'"
    echo ""
    echo "To update a project:"
    echo "  ./network.sh cc invoke -c ${CHANNEL_NAME} -ccn ${CC_NAME} -ccic '{\"function\":\"UpdateDonateProject\",\"Args\":[\"1\",\"UpdatedTitle\",\"UpdatedDescription\",\"newowner\"]}'"
    echo ""
    echo "To delete a project:"
    echo "  ./network.sh cc invoke -c ${CHANNEL_NAME} -ccn ${CC_NAME} -ccic '{\"function\":\"DeleteDonateProject\",\"Args\":[\"1\"]}'"
    echo ""
    echo "To initialize ledger:"
    echo "  ./network.sh cc invoke -c ${CHANNEL_NAME} -ccn ${CC_NAME} -ccic '{\"function\":\"InitLedger\",\"Args\":[]}'"
    echo ""
else
